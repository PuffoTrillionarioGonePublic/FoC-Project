...
inline Container Sha256(const Vec<u8> &v) {
  ...
  auto rv = Container{};
  std::copy(digest.begin(), digest.begin() + kSha256Length, rv.begin());
  return rv;
}


inline auto Mac(const ByteArray auto &v, const ByteArray auto &key)
    -> std::array<u8, kMacLength> {
...
inline bool CertificateIsValid(const Vec<u8> &cert,

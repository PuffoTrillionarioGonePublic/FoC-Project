...
struct CalcSize<Name, Index, Unique,
                decltype(void(adl_ImpListElem(ElemReader<Name, Index>{})))>
    : CalcSize<Name, Index + 1, Unique> {};

template <typename Name, std::size_t Index, typename Unique>
using ReadElem =
    typename decltype(adl_ImpListElem(ElemReader<Name, Index>{}))::type;

template <template <typename...> typename List, typename Name, typename I,
          typename Unique>
struct ReadElemList {};
template <template <typename...> typename List, typename Name, std::size_t... I,
          typename Unique>
...
template <template <typename...> typename List, typename Name,
          typename Unique = DefaultUnique>
using Elems = typename impl::ReadElemList<
    List, Name, std::make_index_sequence<size<Name, Unique>>, Unique>::type;
}  // namespace List
...
#define FILE_ENDPOINT(method_name)                                     \
  ...
  static void method_name(auto self, auto p) { self->method_name(p); } \
  Dummy MAKE_UNIQUE_VARIABLE_NAME(a) =                                 \
      List::PushBack<TypesArray, method_name##Params>{};
...
#define XML_ENDPOINT_NO_ARGS(method_name) \
  XML_ENDPOINT(u8 dummy, (dummy), method_name)
...
struct AbstractController {
  ...
  static bool GenericInvokeCallback(void *self, const ALPHeader &header,
      ...
      mp_for_each<mp_iota_c<N>>([&](auto I) {
        try {
          ...
          if (CurrentType::METHOD_NAME == header.endpoint) {
            if (header.content_type == "text/xml") {
              auto elem =
                  xml::Deserialize<CurrentType>(std::get<std::string>(body));
              elem.callback((SubClass *)self, elem);
            } else if (header.content_type == "binary/file_chunk") {
              auto path = std::filesystem::path{header.file_path.value()};
              if constexpr (std::is_base_of<FileEndpointParams,
                                            CurrentType>::value) {
                CurrentType c{};
                c.path = path;
  ...
  template <typename T>
  static auto GetInvokeCallback(const auto &invoke_function)
      -> std::function<bool(void *, const ALPHeader &, const BufferType &)> {
 ...
 protected:
  ClientInfo &GetClientInfo() { return client_info_; }
 ...
 public:
  explicit AbstractController(std::shared_ptr<IOChannel> io, u64 client_id) {
    ...
    std::clog << "User " << client_info_.username << " with id "
              << client_info_.id << " connected" << std::endl;
    auto generic_invoke_callback_fn = []<typename T>(
                                          void *self, const ALPHeader &header,
                                          const BufferType &body) -> bool {
      return GenericInvokeCallback<T>(self, header, body);
    };
    using TypesArrayWithCallback =
        List::Elems<std::tuple, TypesArray, decltype([] {})>;
    InvokeCallback =
  ...
  void WriteFileChunk(const Vec<u8> &v, size_t offset,

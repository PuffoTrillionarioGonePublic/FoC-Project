...
#define UTIL_FUNC_FOR_SERIALIZATION(archive, ...) BOOST_PP_SEQ_FOLD_LEFT(OP, archive, BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__))
...
#define XML_SER(typed_fields, fields) \
    ...
    struct CONCAT(__AnonStruct, __COUNTER__) : public AnonXmlStruct { \
        ...
        void serialize(const std::variant<std::reference_wrapper<boost::archive::xml_oarchive>, std::reference_wrapper<boost::archive::xml_iarchive>> &__ar, unsigned) { \
            std::remove_const_t<decltype(__ar)>& __ar__ = const_cast<std::remove_const_t<decltype(__ar)&>>(__ar);\
            if (auto pval = std::get_if<std::reference_wrapper<boost::archive::xml_oarchive>>(&__ar__)) \
                UTIL_FUNC_FOR_SERIALIZATION(std::get<0>(__ar__).get(), DEPAREN(fields));  \
            else\
                UTIL_FUNC_FOR_SERIALIZATION(std::get<1>(__ar__).get(), DEPAREN(fields));                                     \
        } \
        friend class boost::serialization::access; \
    } __obj__; xml::Serialize(__obj__);                                         \
    })

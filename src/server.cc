...
struct Controller : public AbstractController<Controller> {
  ...
  void FileDownload(auto p) {
    ...
    while ((rem = file_size - offset) > 0) {
      ...
      WriteFileChunk(v, offset, std::filesystem::path{path}.filename().string());
      offset += chunk_size;
    }
  ...
  /**
   *
   * The client asks to the server the list of the filenames of the available files in his dedicated storage.
   * The client prints to screen the list.
   */
  ...
  XML_ENDPOINT(
      std::string path,
      (path), DeleteFileRequest)

  void DeleteFileRequest(auto p) {
    if (!ValidPath(p.path)) {
      WriteObject(DeleteFileRequestRet{.nonce = "", .error_msg = "path not valid"});
      return;
    }
    GetClientInfo().deletion_nonce = GenerateRandomString(32);
    GetClientInfo().target_path = GetClientInfo().base_path / p.path;
    WriteObject(DeleteFileRequestRet{.nonce = *GetClientInfo().deletion_nonce, .error_msg = ""});
  }

  XML_ENDPOINT(
      std::string nonce;
      std::string res,
      (nonce, res), DeleteFile)

  void DeleteFile(auto p) {
    if (p.res == "y") {
      if (!GetClientInfo().deletion_nonce
  ...
  void Start(const SecVec<u8> &prv_key, const Vec<u8> &certificate) {
    for (;;) {
      ...
      auto th = std::jthread{[this, certificate, prv_key, shared_socket] {
        try {
          auto socket_channel = std::make_shared<SocketChannel>(io_service_, shared_socket);

          auto [io, client_id] =
              SecureDataChannel::ConnectToDataChannelAsServer(socket_channel, prv_key, certificate);
...
int main(int argc, char *argv[]) {
  ...
  auto server_thread = std::jthread{[server, prv_key, certificate] {
    server->Start(prv_key, certificate);
  }};
